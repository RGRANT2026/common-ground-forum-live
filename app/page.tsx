    ...
    <main className="shell">
      ...
      <div className="layout">
        <aside className="sidebar">
          ...
          <div className="sidebar-label live-label"><span>Live spaces</span><span className="live-dot" /></div>
          <button className="live-space-card" onClick={() => setShowLiveRoom(true)}><span className="live-space-art">◉</span><span><strong>Common ground live</strong><small>12 people · hosted by Maya</small></span><b>›</b></button>
        ...
        <aside className="right-rail" id="rules">
          ...
          <section className="rail-card live-card"><div className="live-card-head"><span className="live-badge"><i /> LIVE NOW</span><span>12 listening</span></div><div className="live-orbit"><span>☼</span><span>AR</span><span>JM</span><span>+</span></div><h2>Common ground, live</h2><p>A gentle, hosted conversation about changing your mind without losing face.</p><button className="primary-button" onClick={() => setShowLiveRoom(true)}>Join live space <span>→</span></button></section>
      ...
      {showLiveRoom && <div className="live-room-backdrop"><section className="live-room" role="dialog" aria-modal="true"><div className="live-room-top"><span className="live-badge"><i /> LIVE</span><button onClick={() => setShowLiveRoom(false)} aria-label="Close live space">×</button></div><div className="live-room-title"><p className="eyebrow">COMMON GROUND LIVE</p><h2>Can we disagree<br /><em>without losing face?</em></h2><p>Hosted by Maya Chen · 12 listening</p></div><div className="host-stage"><div className="host-avatar">MC<span className="speaking" /></div><strong>Maya Chen</strong><small>Host · speaking</small></div><div className="listener-grid"><div><span>AR</span><small>You</small></div><div><span>JD</span><small>Jordan</small></div><div><span>SK</span><small>Sam</small></div><div><span>+9</span><small>Listeners</small></div></div><div className="live-room-footer"><button className={isMuted ? "room-control muted" : "room-control"} onClick={() => setIsMuted(!isMuted)}>{isMuted ? "🔇" : "🎙️"}<small>{isMuted ? "Unmute" : "Mute"}</small></button><button className="room-control" onClick={() => notify("Request to speak sent to the host")}>✋<small>Request</small></button><button className="room-control leave" onClick={() => { setShowLiveRoom(false); notify("You left the live space"); }}>↗<small>Leave</small></button></div><div className="room-host-action"><button onClick={() => { setIsLive(!isLive); notify(isLive ? "Live space ended" : "Your live space is ready to host"); }}>{isLive ? "End your live space" : "Host your own live space"} <span>→</span></button></div></section></div>}
